...
import { Frame } from '../Frames/xFrame';
import { getFrameTitle, isValidFrame, getOrderedButtons, isXmtpFrame } from '../Frames/FrameInfo';
...
export const MessageItem = ({ message, senderAddress, client }) => {
  ...
  const handleFrameButtonClick = async (buttonIndex, action = 'post') => {
    try {
      ...
      if (action === 'tx') {
        const transactionInfo = await framesClient.proxy.postTransaction(button.target, payload);
        ...
        if (transactionInfo.method === 'eth_personalSign') {
          ...
          const completeTransactionMetadata = await framesClient.proxy.post(button.postUrl, payloadWithTxId);
          ...
          try {
            ...
            const buttonPostUrl = frameMetadata.extractedTags['fc:frame:button:1:post_url'];
            const completeTransactionMetadata = await framesClient.proxy.post(buttonPostUrl, {
              ...payload,
              transactionId: hash,
            });
            setFrameMetadata(completeTransactionMetadata);
        ...
        const { redirectedTo } = await framesClient.proxy.postRedirect(postUrl, payload);
  ...
  const renderMessage = (message) => {
    ...
    if (frameMetadata?.url && showFrame) content = content.replace(frameMetadata?.url, '');
    if (!codec) {
  ...
  return (
    <li style={isSender ? styles.senderMessage : styles.receiverMessage} key={message.id}>
