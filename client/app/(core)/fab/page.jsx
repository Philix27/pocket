...
import { MessageContainer } from './xMessageContainer';
...
export default function FloatingInbox({
  ...
  useEffect(() => {
    const initialIsOpen = isPWA || isContained || localStorage.getItem('isWidgetOpen') === 'true' || false;
    const initialIsOnNetwork = localStorage.getItem('isOnNetwork') === 'true' || false;
    const initialIsConnected = (localStorage.getItem('isConnected') && wallet === 'true') || false;
  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (!signer && (isConnected || onboarding || localStorage.getItem('walletAddress') !== null)) {
        connectWallet();
      }
    };
    init();
              ...
              <div style={{ flex: 1, textAlign: 'right' }}>
                ...
                {isFullScreen && (
                  ...
                  >
                    ...
                    {'Your address: ' + address?.substring(0, 7) + '...' + address?.substring(address?.length - 5)} 📋
                  </div>
                )}
              </div>
            </div>
