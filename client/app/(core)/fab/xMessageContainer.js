...
import { MessageInput } from './xMessageInput';
...
export const MessageContainer = ({
  ...
  const updateMessages = (prevMessages, newMessage) => {
    const doesMessageExist = prevMessages.some((existingMessage) => existingMessage.id === newMessage.id);
        ...
        <>
          {isFullScreen && (
            <div style={styles.peerAddressContainer}>
              <div style={styles.peerAddressContainerLabel}>
                ...
                <div
                  onClick={() => {
                    window.open(window.location.href + 'dm/' + conversation.peerAddress, '_blank');
                  }}
          ...
          <ul style={styles.messagesList}>
            {messages.slice().map((message) => {
              return (
                <MessageItem key={message.id} message={message} senderAddress={message.senderAddress} client={client} />
              );
            })}
          ...
          {isConsent && showPopup ? (
            <div style={styles.popup}>
              <h4 style={styles.popupTitle}>
                State is <small>{conversation.consentState}</small>. Do you trust this contact?
              </h4>
              <div style={styles.popupInner}>
                {conversation.consentState !== 'allowed' && (
                  <button style={{ ...styles.popupButton, ...styles.acceptButton }} onClick={handleAccept}>
                    Allow
                  </button>
                )}
                {conversation.consentState !== 'denied' && (
                  <button style={{ ...styles.popupButton, ...styles.denyButton }} onClick={handleDeny}>
                    Deny
                  </button>
                )}
              </div>
